...
import uk.gov.hmrc.agentsubscriptionfrontend.models.{AgentSession, AuthProviderId, BusinessAddress}
...
class BusinessIdentificationControllerISpecIt extends BaseISpecIt {
  ...
  "POST /change-business-name changeBusinessName" should {
    ...
    "redirect to /progress-saved when user inputs valid data and saves" in new Setup {
      ...
      redirectLocation(result) shouldBe Some(
        routes.TaskListController.savedProgress(backLink = Some(routes.BusinessIdentificationController.changeBusinessName().url)).url
      )
    }
  }
  ...
  "POST /change-business-email changeBusinessEmail" should {
    ...
    "redirect to /progress-saved when user inputs valid data and saves" in new Setup {
      ...
      redirectLocation(result) shouldBe Some(
        routes.TaskListController.savedProgress(backLink = Some(routes.BusinessIdentificationController.changeBusinessEmail().url)).url
      )
    }
  }
