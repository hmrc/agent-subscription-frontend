...
class VatDetailsControllerISpecIt extends BaseISpecIt with SessionDataMissingSpec {
  ...
  "GET /registered-for-vat" should {
    ...
    "display /registered-for-vat if nino doesn't exist from auth" in new TestSetupNoJourneyRecord {
      ...
      val result: Result = await(controller.showRegisteredForVatForm()(request))
    ...
    "redirect to /date-of-birth when user has assured their nino and not DOB yet" in new TestSetupNoJourneyRecord {
      ...
      await(
        sessionStoreService.cacheAgentSession(
          AgentSession(businessType = Some(SoleTrader), nino = Some("AE123456C"), dateOfBirthFromCid = Some(DateOfBirth(dob)))
    ...
    "pre-populate the registeredForVat if one is already stored in the session" in new TestSetupNoJourneyRecord {
      ...
      await(
        sessionStoreService.cacheAgentSession(
          AgentSession(
            ...
            dateOfBirthFromCid = Some(DateOfBirth(dob)),
      ...
      val link: Element = doc.getElementById("registeredForVat")
  ...
  "POST /registered-for-vat" should {
    "handle valid form and redirect when choice is yes" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] =
        authenticatedAs(subscribingAgentEnrolledForNonMTD, POST).withFormUrlEncodedBody("registeredForVat" -> "yes")
      sessionStoreService.currentSession.agentSession = Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now()))))

      val result: Result = await(controller.submitRegisteredForVatForm()(request))
      status(result) shouldBe 303
    ...
    "handle valid form and redirect when choice is no" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] =
        authenticatedAs(subscribingAgentEnrolledForNonMTD, POST).withFormUrlEncodedBody("registeredForVat" -> "no")
      sessionStoreService.currentSession.agentSession = Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now()))))
    ...
    "handle form with errors" in new TestSetupNoJourneyRecord {
      ...
      result should containMessages("registered-for-vat.title", "button.yes", "button.no", "registered-for-vat.error.no-radio-selected")
    }
  }
  ...
  "GET /vat-registration-details" should {
    "display the page with expected content" in new TestSetupNoJourneyRecord {
      ...
      val result: Result = await(controller.showVatDetailsForm()(request))
    ...
    "pre-populate the vatDetails if one is already stored in the session" in new TestSetupNoJourneyRecord {
      val vrd: LocalDate = LocalDate.of(2010, 1, 1)
      implicit val request: FakeRequest[AnyContentAsEmpty.type] = authenticatedAs(subscribingAgentEnrolledForNonMTD)
      await(
        sessionStoreService.cacheAgentSession(
          AgentSession(Some(SoleTrader), vatDetails = Some(VatDetails(Vrn("688931961"), vrd)))
        )(request, global, aesCrypto)
      )
      ...
      var link: Element = doc.getElementById("vrn")
  ...
  "POST /vat-registration-details" should {
    ...
    "handle valid forms, check vat known facts, store data in session and redirect" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] = authenticatedAs(subscribingAgentEnrolledForNonMTD, POST)
        .withFormUrlEncodedBody("vrn" -> "688931961", "regDate.day" -> "1", "regDate.month" -> "11", "regDate.year" -> "2010")
      sessionStoreService.currentSession.agentSession =
        Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now())), registeredForVat = Some("Yes")))

      AgentSubscriptionStub.withMatchingVrnAndDateOfReg(Vrn("688931961"), LocalDate.of(2010, 11, 1))

      val result: Result = await(controller.submitVatDetailsForm()(request))
      status(result) shouldBe 303
      redirectLocation(result) shouldBe Some(routes.BusinessIdentificationController.showConfirmBusinessForm().url)
    ...
    "handle forms with missing vrn" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] =
        authenticatedAs(subscribingAgentEnrolledForNonMTD, POST).withFormUrlEncodedBody("vrn" -> "")
      sessionStoreService.currentSession.agentSession =
        Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now())), registeredForVat = Some("Yes")))
    ...
    "handle forms containing vrn with invalid text" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] =
        authenticatedAs(subscribingAgentEnrolledForNonMTD, POST).withFormUrlEncodedBody("vrn" -> "blah")
      sessionStoreService.currentSession.agentSession =
        Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now())), registeredForVat = Some("Yes")))
    ...
    "handle forms with missing vat registration date" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] = authenticatedAs(subscribingAgentEnrolledForNonMTD, POST)
        .withFormUrlEncodedBody("regDate.day" -> "", "regDate.month" -> "", "regDate.year" -> "")
      sessionStoreService.currentSession.agentSession =
        Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now())), registeredForVat = Some("Yes")))
    ...
    "handle forms with very old vat registration date < 1900" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] = authenticatedAs(subscribingAgentEnrolledForNonMTD, POST)
        .withFormUrlEncodedBody("regDate.day" -> "1", "regDate.month" -> "1", "regDate.year" -> "1010")
      sessionStoreService.currentSession.agentSession =
        Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now())), registeredForVat = Some("Yes")))
    ...
    "handle forms with future dated vat registration" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] = authenticatedAs(subscribingAgentEnrolledForNonMTD, POST)
        .withFormUrlEncodedBody("regDate.day" -> "1", "regDate.month" -> "1", "regDate.year" -> "3010")
      sessionStoreService.currentSession.agentSession =
        Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now())), registeredForVat = Some("Yes")))
    ...
    "handle forms with vat registration date as non-digits" in new TestSetupNoJourneyRecord {
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] = authenticatedAs(subscribingAgentEnrolledForNonMTD, POST)
        .withFormUrlEncodedBody("regDate.day" -> "qq", "regDate.month" -> "we", "regDate.year" -> "erd")
      sessionStoreService.currentSession.agentSession =
        Some(agentSession.copy(dateOfBirth = Some(DateOfBirth(LocalDate.now())), registeredForVat = Some("Yes")))
