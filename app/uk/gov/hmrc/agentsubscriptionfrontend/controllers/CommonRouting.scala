...
class CommonRouting @Inject()(
  ...
  private def continueFromNationalInsurancePage(agentSession: AgentSession) = {
    agentSession match {
      ...
      case _ if agentSession.companyRegistrationNumber.isEmpty => routes.RegisteredForVatController.showRegisteredForVatForm()
    }
  }

  private def continueFromCompanyRegistrationPage(agentSession: AgentSession) = {
    agentSession match {
      case _  if agentSession.companyRegistrationNumber.isEmpty => routes.BusinessIdentificationController.showCompanyRegNumberForm()
    }
  }
}
